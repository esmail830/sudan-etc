    ...
    try {
      ...
      if (image != null) {
        try {
          ...
          await _supabase.storage.from('report_images').upload(fileName, image);
      ...
      // استدعاء المصنف لتحديد الفني المسؤول (البلاغ الخاص)
      ...
      try {
        ...
        // ignore: avoid_print
        print(
          '[Classifier] Assigned label (private): ' + (result?.label ?? 'null'),
        );
        if (result != null && result.label.isNotEmpty) {
          assignedTechnician = result.label;
          print(
            '[SupabaseService] Using classifier result: $assignedTechnician',
          );
        } else {
          // إذا فشل Classifier، استخدم نوع العطل كتصنيف افتراضي
          assignedTechnician = type;
          print(
            '[SupabaseService] Classifier failed, using fallback: $assignedTechnician',
          );
        }
      } catch (e) {
        ...
        assignedTechnician = type;
        print(
          '[SupabaseService] Classifier error, using fallback: $assignedTechnician',
        );
        print('[SupabaseService] Error details: $e');
      }

      // إذا كان المستخدم مسجل دخول، امنع إرسال أكثر من بلاغ خاص واحد غير مكتمل
      ...
      if (currentUserId != null) {
        ...
        if (existing.isNotEmpty) {
          return {
            'success': false,
            'message': 'لديك بلاغ خاص قيد المعالجة بالفعل',
          };
        }
      }
      ...
      // ignore: avoid_print
      print('[SupabaseService] Payload for reportc: ' + reportData.toString());
      ...
      final response =
          await _supabase.from('reportc').insert(reportData).select().single();

      print('تم الإدراج بنجاح: $response');

      // إشعار محلي داخل التطبيق للمستخدم
      ...
      if (error.toString().contains('relation "reportc" does not exist')) {
        errorMessage =
